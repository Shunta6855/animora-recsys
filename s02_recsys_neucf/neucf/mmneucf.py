...
class MultiModalNeuCF(nn.Module):
    def __init__(self, config, image_feature_dim, text_feature_dim):
        ...
        """
        super(MultiModalNeuCF, self).__init__()
        self.config = config
        ...
        self.fusion_layer_mlp = nn.Linear(
            self.latent_dim_mlp + image_emb_dim + text_emb_dim, self.latent_dim_mlp
        )
        ...
        self.fusion_layer_mf = nn.Linear(
            self.latent_dim_mf + image_emb_dim + text_emb_dim, self.latent_dim_mf
        )
    ...
    def forward(self, user_indices, item_indices, image_features, text_features):
        ...
        image_emb = F.relu(
            self.fc_image(image_features)
        )  # shape: (batch_size, image_emb_dim)
        text_emb = F.relu(
            self.fc_text(text_features)
        )  # shape: (batch_size, text_emb_dim)

        # ----------------------------------
        # MLP Layers
        # ----------------------------------
        user_emb_mlp = self.embedding_user_mlp(
            user_indices
        )  # shape: (batch_size, latent_dim_mlp)
        item_emb_mlp = self.embedding_item_mlp(
            item_indices
        )  # shape: (batch_size, latent_dim_mlp)

        # 融合: 投稿ID埋め込み、画像特徴、テキスト特徴を連結し、全結合層で変換
        fused_item_mlp = torch.cat([item_emb_mlp, image_emb, text_emb], dim=-1)
        fused_item_mlp = F.relu(
            self.fusion_layer_mlp(fused_item_mlp)
        )  # shape: (batch_size, latent_dim_mlp)
        ...
        # GMF Layers
        # ----------------------------------
        user_emb_mf = self.embedding_user_mf(
            user_indices
        )  # shape: (batch_size, latent_dim_mf)
        item_emb_mf = self.embedding_item_mf(
            item_indices
        )  # shape: (batch_size, latent_dim_mf)

        # 融合: 投稿ID埋め込み、画像特徴、テキスト特徴を連結し、新たな投稿表現を構築
        fused_item_mf = torch.cat([item_emb_mf, image_emb, text_emb], dim=-1)
        fused_item_mf = F.relu(
            self.fusion_layer_mf(fused_item_mf)
        )  # shape: (batch_size, latent_dim_mf)
    ...
    def finetuning(self, state_dict, new_config):
        ...
        def expand_embedding(old_emb, new_size):
            ...
            if new_size <= old_size:
                print(
                    f"新しいサイズ({new_size})が古いサイズ({old_size})以下なので、重みをそのまま読み込みます"
                )
        ...
        self.embedding_user_mlp = expand_embedding(
            self.embedding_user_mlp, new_config["num_users"]
        )
        self.embedding_item_mlp = expand_embedding(
            self.embedding_item_mlp, new_config["num_items"]
        )
        self.embedding_user_mf = expand_embedding(
            self.embedding_user_mf, new_config["num_users"]
        )
        self.embedding_item_mf = expand_embedding(
            self.embedding_item_mf, new_config["num_items"]
        )
        ...
        print(
            f"重みを拡張して読み込みました: ユーザー数({new_config['num_users']}), アイテム数({new_config['num_items']})"
        )


class MultiModalNeuCFEngine(Engine):
    def __init__(self, config):
        self.config = config
        self.model = MultiModalNeuCF(
            config, config["image_feature_dim"], config["text_feature_dim"]
        )
        if config["use_cuda"] is True:
            use_cuda(True, config["device_id"])
            self.model.cuda()
        super(MultiModalNeuCFEngine, self).__init__(config)
        print(self.model)
