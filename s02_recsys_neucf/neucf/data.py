...
class MultiModalDataset(Dataset):
    ...
    def __init__(
        self, user_tensor, item_tensor, target_tensor, image_tensor, text_tensor
    ):
...
class SampleGenerator(object):
    def __init__(self, ratings):
        ...
        filtered = self.preprocess_ratings.groupby("user_id").filter(
            lambda x: len(x) >= 2
        )
    ...
    def _split_loo(self, ratings):
        ...
        """
        ratings["rank_latest"] = ratings.groupby(["user_id"])["created_at"].rank(
            method="first", ascending=False
        )
        test = ratings[ratings["rank_latest"] == 1]
        train = ratings[ratings["rank_latest"] > 1]
        train = train[
            ["user_id", "post_id", "rating", "image_feature", "text_feature"]
        ]  # 画像・テキスト特徴を追加
    ...
    def _sample_negative(self, ratings):
        ...
        """
        interact_status = (
            ratings.groupby("user_id")["post_id"]
            .apply(set)
            .reset_index()
            .rename(columns={"post_id": "interacted_items"})
        )
        ...
        interact_status["negative_samples"] = interact_status["negative_items"].apply(
            lambda x: random.sample(list(x), min(len(x), 99))
        )
    ...
    def instance_a_train_loader(self, num_negatives, batch_size):
        ...
        train_ratings["negatives"] = train_ratings["negative_items"].apply(
            lambda x: random.sample(list(x), min(len(x), num_negatives))
        )
